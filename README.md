...
当前版本为 `2.1.0`。Windows x64 交付完整离线 Setup；Linux x86_64 同步交付 runtime-complete AppImage、现代发行版 deb 与 UOS Desktop 20 专用 deb。升级时直接安装新版完整包，用户工作区数据保持在原数据目录。原有 PySide6 客户端已冻结，只作为 `.rpaz` v1 行为和迁移参考；新功能只进入 Tauri + React + Rust 架构。
