...
- 内置 DRPA Local Dify：管理 Chat/Completion 应用和 OpenAI-compatible Provider，提供流式 Markdown 调试、SQLite 运行轨迹、Dify YAML DSL 导入导出、云端映射检查，以及带独立应用 Token 的本地 Dify Service API；`dify-loves-hermes` 可继续把本地应用转换成 OpenAI 接口。
...
- Local Dify 第一阶段执行 Chat 与 Completion；可导入并保留 Chatflow/Workflow DSL，但可视化 Workflow 画布、知识检索与复杂节点执行器属于后续阶段。
