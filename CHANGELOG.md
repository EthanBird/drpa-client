...
- 内置 RPAZ Agent 增加单轮命令幂等账本：模型连续提交相同工具与参数时复用首次结果，不再真实执行第二次；工具结果同时显式记录执行状态、是否产生输出及是否沿用历史结果，空 stdout/stderr 不再被误判为尚未运行。
- JCode 开发者 Agent 接入 `pre_tool`/`post_tool` 生命周期守卫，在命令进入 sidecar 执行器之前拦截同一状态屏障内的重复调用；文件变更、新会话轮次或上次执行失败后仍可正常重试。
- JCode sidecar 更新至 `0.79.1`，下载脚本和离线打包锁同步更新官方归档 SHA-256。
