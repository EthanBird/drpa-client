...
export function LibraryPage() {
  ...
  const install = async () => {
    ...
    try {
      ...
      const selected = await open({ multiple: false, filters: [{ name: "DRPA RPAZ 包", extensions: ["rpaz"] }] });
      if (!selected) return;
  ...
  return (
    <div className="page library-page">
      <header className="page-header"><div><div className="eyebrow">RPAZ 包管理</div><h1>RPAZ 包</h1><p>安装、检查和管理本地可执行的 `.rpaz` 自动化包。</p></div><button className="button primary" type="button" onClick={install} disabled={installing}><Plus size={15} /> {installing ? "正在安装…" : "安装 RPAZ 包"}</button></header>
      {notice && <div className={notice.startsWith("安装成功") ? "operation-notice success" : "operation-notice error"}>{notice}</div>}
      <div className="library-toolbar"><div className="large-search"><Search size={16} /><input aria-label="搜索 RPAZ 包" placeholder="搜索名称、ID 或运行环境" value={query} onChange={(event) => setQuery(event.target.value)} /></div><div className="view-toggle"><button className={view === "grid" ? "active" : ""} type="button" aria-label="网格视图" onClick={() => setView("grid")}><Grid2X2 size={14} /></button><button className={view === "list" ? "active" : ""} type="button" aria-label="列表视图" onClick={() => setView("list")}><List size={15} /></button></div></div>
      {packages.length === 0 && <div className="empty-state"><Box size={28} /><h2>{query ? "没有匹配的 RPAZ 包" : "尚未安装 RPAZ 包"}</h2><p>{query ? "请调整搜索条件。" : "可以安装示例 `.rpaz`，或在开发工作室中新建项目。"}</p><button className="button secondary" type="button" onClick={() => setActiveNavigation("studio")}>新建项目</button></div>}
      ...
      {menu && <div className="context-menu" role="menu" style={{ left: Math.min(menu.x, window.innerWidth - 210), top: Math.min(menu.y, window.innerHeight - 150) }} onClick={(event) => event.stopPropagation()}>
        ...
        <span />
        <button className="danger" type="button" role="menuitem" onClick={() => { void uninstall(menu.packageId); setMenu(null); }}><Trash2 size={14} /> 卸载 RPAZ 包</button>
      </div>}
    </div>
  );
}
