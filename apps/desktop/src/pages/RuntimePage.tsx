...
import type { PlatformCapabilities, RuntimeStatus } from "../domain/models";
  ...
  return (
    <div className="page runtime-page">
      <header className="page-header">
        <div><div className="eyebrow">{platform ? `${platform.displayName} 离线运行基础设施` : "离线运行基础设施"}</div><h1>运行环境</h1><p>这里展示的是真实发行内容和健康状态，不依赖系统 Python。</p></div>
      ...
      {status && <div className="runtime-grid">
        ...
        <RuntimeCard icon={<Database size={18} />} title="Jupyter Kernel" value="ipykernel · jupyter_client · ZMQ" detail="真实 Jupyter 消息协议，环境位于可写应用数据目录" path={status.environmentRoot} />
      ...
      {repairConfirmationOpen && <div className="knowledge-confirm-overlay" role="dialog" aria-modal="true" aria-label="确认强制重建运行环境" onMouseDown={() => setRepairConfirmationOpen(false)}>
        <section className="knowledge-confirm" onMouseDown={(event) => event.stopPropagation()}>
          ...
          <div><h2>强制重建整个运行环境？</h2><p>该操作会停止 Studio Kernel，删除并离线重建应用数据目录中的 runtime-environment，通常需要数分钟。脚本包、项目和运行产物会保留。</p></div>
