...
- 使用中间列表管理任务配置，类似文件管理器中的“任务文件”。
- 选中脚本包后，在右侧显示脚本包名称、描述、ID 和 runtime。
- 根据 manifest 参数生成可编辑参数表，用于展示参数名、类型、必填、默认值、当前值和说明。
- 根据 manifest 参数生成运行表单，用于实际填写运行参数；表单与参数表“当前值”双向同步。
- 可以把当前参数保存为任务配置。
- 可以选择任务配置并启动运行。
- 可以启动多个运行实例，并在运行实例表中切换查看日志。
- 支持基础参数类型：
  - string
  - password
  - boolean
...
- 中间用于管理“保存好的任务配置”。
- 右侧用于展示“如何运行”和“运行结果”。
- 右侧参数详情使用滚动容器，参数多时不会把日志区和底部按钮挤出屏幕。
- 参数表自身限制高度并提供表内滚动，说明列自适应剩余宽度。
...
- 参数表：展示参数名、类型、必填、默认值、当前值、说明，帮助用户理解任务配置；其中“当前值”可直接编辑。
- 运行表单：根据类型生成可编辑控件，运行时收集为 `ctx.params`；它和参数表当前值双向同步。
