...
> **设计草案，尚未实装到 DRPA Next。** 文中的 PySide6 组件属于旧版参考。后续实现必须接入 Tauri/Rust Host 的 capability、路径校验、运行审计和离线资产策略，并以 [`ROADMAP.md`](ROADMAP.md) 的阶段门禁为准。

本文档设计 DRPA Client 的浏览器录制功能。目标是把用户在浏览器中的操作录制为结构化事件，再生成一个可运行但需要人工修订的 `.rpaz` 草稿脚本包。

浏览器录制属于高级功能，默认不显示在主导航中。用户需要在“设置”中开启“启用高级功能：浏览器录制”后才会看到入口。
