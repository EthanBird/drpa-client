...
DRPA Next `2.0.4` 正式提供 Linux x86_64 runtime-complete AppImage、现代发行版 deb 与 UOS Desktop 20 专用 deb，与 Windows Setup 共用 `desktop-v2.0.4` Release。Ubuntu 22.04 workflow 负责构建 sealed runtime、三种格式打包、解包验证、断网 bootstrap、deb 安装卸载、Dify2API 健康检查和 X11 启动后上传；UOS 包还必须在 Debian 10/glibc 2.28 容器中通过完整安装与运行门禁。Ubuntu 24.04、真实 UOS 20、Wayland 和人工 GUI 验收仍是持续回归项，不能因容器测试通过而删除这些门禁。
...
python tools/linux/build_bundled_deb.py \
  ...
  --output drpa-next-2.0.4-linux-x86_64.deb \
...
```bash
chmod +x path/to/DRPA-Next_2.0.4_amd64.AppImage
DRPA_DATA_DIR="$PWD/.drpa-appimage-data" \
  path/to/DRPA-Next_2.0.4_amd64.AppImage
```
...
```bash
sudo apt install ./drpa-next-2.0.4-linux-x86_64.deb
...
python tools/linux/build_uos20_deb.py \
  ...
  --output drpa-next-2.0.4-linux-x86_64-uos20.deb \
  --work-dir "$extract_root/uos20-deb-work"

python tools/linux/verify_uos20_deb.py \
  --deb drpa-next-2.0.4-linux-x86_64-uos20.deb \
  --expected-version '2.0.4-1+uos20.3' \
  --extract-root "$extract_root/uos20-verify" \
  --manifest-output drpa-next-2.0.4-linux-x86_64-uos20-deb-manifest.json
```
...
./.venv/bin/python tools/offline/validate_requirements.py offline/requirements/runtime.txt
./.venv/bin/python tools/release/check_version_consistency.py --expected 2.0.4
```
...
正式 deb 都不应要求该系统包。若 `apt` 仍提示安装 `libwebkit2gtk-4.1-0`，先用 `dpkg-deb -f <包> Version Depends` 检查：现代包版本为 `2.0.4-1`，UOS 包版本为 `2.0.4-1+uos20.3`；UOS 用户还必须确认文件名包含 `uos20`。出现 `1.0.0`、`uos20.1` 或 `uos20.2` 说明仍在使用已被 Release 覆盖的旧 deb；`uos20.1` 可能永久白屏，`uos20.2` 的 Deepin 验证截图没有任何文字。
...
- Windows 与 Linux/UOS 稳定基线：`desktop-v2.0.4`，Release 同时包含 Windows x64 Setup、AppImage、现代 deb 与 UOS 20 专用 deb。
